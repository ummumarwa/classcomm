...
class RegistrationManager(models.Manager):
    ...
    def resend_activation(self, email, site):
        """
        Resend activation e-mail for accounts that have not expired
        for Users with paramter email.
        
        """
        # In our system We requrie registering E-mails to be unique
        # and only create 1 registration_profile per User so this
        # Code will find the registration_profile by User.email
        ...
        for user in users:
            ...
            for registration_profile in registration_profiles:
                if not registration_profile.has_activated():
                    if registration_profile.activation_key_expired():
                        registration_profile.reset_activation()
                    registration_profile.send_activation_email(site)
                    return True              
        return False
    # End Def
...
class RegistrationProfile(models.Model):
    ...
    def has_activated(self):
        """
        Returns True or False depending if this User
        has activated successfully or not.
